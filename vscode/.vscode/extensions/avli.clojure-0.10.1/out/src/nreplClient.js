...
const evaluate = (code, session) => clone(session).then((session_id) => {
    const msg = { op: 'eval', code: code, session: session_id };
    return send(msg);
});
const evaluateFile = (code, filepath, session) => clone(session).then((session_id) => {
...
const clone = (session) => send({ op: 'clone', session: session }).then(respObjs => respObjs[0]['new-session']);
